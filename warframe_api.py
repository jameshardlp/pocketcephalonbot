...
class NotificationScheduler:
    ...
    async def check_invasions(self):
        ...
        if data:
            # Создаем хэш на основе ID, прогресса и составов
            invasions_hash = hash(str([(inv['id'], inv['completion'], inv.get('attacker', {}).get('name'), inv.get('defender', {}).get('name')) for inv in data]))
            key = 'invasions_hash'
